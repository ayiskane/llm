    ...
    <div className="flex gap-2">
      <div className="relative flex-1">
        ...
        <input
          ...
          className={cn("w-full rounded-xl pl-11 pr-10 py-3 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40", surface.control, border.visible, text.body)}
        />
        {value && (
          <button onClick={onClear} className={cn("absolute right-4 top-1/2 -translate-y-1/2", text.hint, "hover:text-slate-200")}>
      ...
      <button
        ...
        className={cn(
          ...
          hasActiveFilters ? 'bg-blue-500/20 border-blue-500/50 text-blue-400' : cn(surface.control, border.visible, text.hint)
        )}
      >
...
function FilterModalContent({ filters, onFilterChange }: {
  ...
  return (
    <div className="space-y-6">
      {/* Region */}
      <div>
        <label className={cn("text-xs uppercase tracking-wider font-medium mb-3 block", text.hint)}>Region</label>
            ...
            <button
              ...
              className={cn(
                ...
                filters.region === r.id
                  ...
                  : cn(surface.modal, text.label, border.visible, 'hover:border-slate-600')
              )}
            >
      ...
      {/* Jurisdiction */}
      <div>
        <label className={cn("text-xs uppercase tracking-wider font-medium mb-3 block", text.hint)}>Jurisdiction</label>
        <div className="flex flex-wrap gap-2">
...
function CentreListItem({ centre, onClick }: { centre: CorrectionalCentre; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className={cn("w-full text-left px-4 py-3 last:border-b-0", border.divider, surface.cardHover, "active:bg-slate-800/50")}>
      <div className={cn("text-sm font-medium mb-1.5", text.body)}>{centre.name}</div>
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className={cn("px-2 py-1 rounded text-[9px] font-mono leading-none inline-flex items-center gap-1 uppercase tracking-widest", surface.card, border.visible, text.hint)}>
...
function LetterSection({ letter, centres, onCentreClick }: {
  ...
  return (
    <div id={`section-${letter}`} data-letter={letter}>
      <div className={cn("sticky top-0 z-10 px-4 py-2 border-b border-slate-800/50", surface.page)}>
...
export function CorrectionsIndexPage({ initialCentres }: CorrectionsIndexPageProps) {
  ...
  return (
    <div className={cn("h-full flex flex-col", surface.page)}>
      {/* Header */}
      <div className={cn("shrink-0 border-b border-slate-800/50", surface.header)}>
        <div className="px-4 pt-4 pb-2">
          <h1 className={cn("text-xl font-bold", text.heading)}>BC Corrections Index</h1>
        </div>
        ...
        <div ref={scrollContainerRef} className="h-full overflow-y-auto">
          {groupedCentres.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 px-4">
              <FaBuildingShield className={cn("w-12 h-12 mb-4", text.disabled)} />
              <p className={cn("text-center", text.hint)}>{searchQuery ? `No centres found for "${searchQuery}"` : 'No centres match your filters'}</p>
            ...
            <>
              ...
              <div className="py-4 text-center">
                <span className={cn("text-xs", text.placeholder)}>{filteredCentres.length} {filteredCentres.length === 1 ? 'centre' : 'centres'}</span>
              </div>
            </>
          )}
        </div>
