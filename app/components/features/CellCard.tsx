...
import { Telephone, TelephoneFill, Clipboard, Check2 } from 'react-bootstrap-icons';
...
function CopyButton({ text, className }: CopyButtonProps) {
  ...
  return (
    ...
    >
      {copied ? (
        <Check2 className="w-3 h-3 text-green-400" />
      ) : (
...
function CallButton({ phone, className }: CallButtonProps) {
  ...
  return (
    <button
      ...
      title="Call"
    >
      <TelephoneFill className="w-3 h-3 text-green-400" />
    </button>
  );
}
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center", iconBg)}>
        <Telephone className={cn("w-5 h-5", iconColor)} />
      </div>
