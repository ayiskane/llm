/**
 * FontAwesome Pro 7.1.0 Icons - Unified Icon Library
 * All icons using exact SVG paths from FA Pro 7.1.0
 *
 * Styles used:
 * - Regular (navigation, misc)
 * - Solid (spinner, police)
 * - Jelly Regular (at, eye, sliders, location, circle-check)
 * - Jelly Duo Regular (comment-dots)
 * - Slab Press Regular (video, phone, calendar)
 * - Utility Semibold (circle-exclamation)
 */

import { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement>;

// =============================================================================
// NAVIGATION ICONS (regular)
// =============================================================================

// arrow-left (regular) - 512×512
export function FaArrowLeft(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M7 239c-9.4 9.4-9.4 24.6 0 33.9L175 441c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9L81.9 280 488 280c13.3 0 24-10.7 24-24s-10.7-24-24-24L81.9 232 209 105c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0L7 239z" />
    </svg>
  );
}

// chevron-down (regular) - 448×512
export function FaChevronDown(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      fill="currentColor"
      {...props}
    >
      <path d="M207.5 409c9.4 9.4 24.6 9.4 33.9 0l200-200c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-183 183-183-183c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9l200 200z" />
    </svg>
  );
}

// chevron-right (regular) - 320×512
export function FaChevronRight(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 320 512"
      fill="currentColor"
      {...props}
    >
      <path d="M313.5 239c9.4 9.4 9.4 24.6 0 33.9l-200 200c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l183-183-183-183c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l200 200z" />
    </svg>
  );
}

// =============================================================================
// SEARCH & FILTER ICONS
// =============================================================================

// magnifying-glass (regular) - 512×512
export function FaMagnifyingGlass(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M368 208a160 160 0 1 0 -320 0 160 160 0 1 0 320 0zM337.1 371.1C301.7 399.2 256.8 416 208 416 93.1 416 0 322.9 0 208S93.1 0 208 0 416 93.1 416 208c0 48.8-16.8 93.7-44.9 129.1L505 471c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0L337.1 371.1z" />
    </svg>
  );
}

// xmark (regular) - 384×512
export function FaXmark(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 384 512"
      fill="currentColor"
      {...props}
    >
      <path d="M7.5 105c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l151 151 151-151c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9l-151 151 151 151c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-151-151-151 151c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l151-151-151-151z" />
    </svg>
  );
}

// sliders (jelly-regular) - 560×560
export function FaSliders(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path d="M192 280c24.5 0 49.2-5 66.7-23.9 16.9-18.3 21.3-43.6 21.3-69.5l0-93.3C280 67.5 275.6 42.2 258.7 23.9 241.2 5 216.5 0 192 0L172 0C147.5 0 122.8 5 105.3 23.9 88.4 42.2 84 67.5 84 93.3l0 18.7-56 0c-15.5 0-28 12.5-28 28s12.5 28 28 28l56 0 0 18.7c0 25.8 4.4 51.2 21.3 69.5 17.5 18.9 42.2 23.9 66.7 23.9l20 0zm-52-93.3l0-93.3c0-21.6 4.1-29 6.4-31.4 1.8-1.9 7.1-5.9 25.6-5.9l20 0c18.5 0 23.8 4 25.6 5.9 2.3 2.5 6.4 9.8 6.4 31.4l0 93.3c0 21.6-4.1 29-6.4 31.4-1.8 1.9-7.1 5.9-25.6 5.9l-20 0c-18.5 0-23.8-4-25.6-5.9-2.3-2.5-6.4-9.8-6.4-31.4zM388 560c24.5 0 49.2-5 66.7-23.9 16.9-18.3 21.3-43.6 21.3-69.5l0-18.7 56 0c15.5 0 28-12.5 28-28s-12.5-28-28-28l-56 0 0-18.7c0-25.8-4.4-51.2-21.3-69.5-17.5-18.9-42.2-23.9-66.7-23.9l-20 0c-24.5 0-49.2 5-66.7 23.9-16.9 18.3-21.3 43.6-21.3 69.5l0 93.3c0 25.8 4.4 51.2 21.3 69.5 17.5 18.9 42.2 23.9 66.7 23.9l20 0zm32-93.3c0 21.6-4.1 29-6.4 31.4-1.8 1.9-7.1 5.9-25.6 5.9l-20 0c-18.5 0-23.8-4-25.6-5.9-2.3-2.5-6.4-9.8-6.4-31.4l0-93.3c0-21.6 4.1-29 6.4-31.4 1.8-1.9 7.1-5.9 25.6-5.9l20 0c18.5 0 23.8 4 25.6 5.9 2.3 2.5 6.4 9.8 6.4 31.4l0 93.3zM0 420c0-15.5 12.5-28 28-28l182 0c15.5 0 28 12.5 28 28s-12.5 28-28 28L28 448c-15.5 0-28-12.5-28-28zM350 112c-15.5 0-28 12.5-28 28s12.5 28 28 28l182 0c15.5 0 28-12.5 28-28s-12.5-28-28-28l-182 0z" />
    </svg>
  );
}

// =============================================================================
// SECTION HEADER / NAV ICONS
// =============================================================================

// at (jelly-regular) - 560×560
export function FaAt(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path d="M116 116c-37.6 37.6-60 93-60 164s22.5 126.4 60 164 93 60 164 60c64.1 0 115.4-18.3 152.4-49.4 11.8-10 29.5-8.4 39.5 3.4s8.4 29.5-3.4 39.5c-48.7 41-113.5 62.6-188.4 62.6-83 0-153.6-26.5-203.5-76.5S0 363 0 280 26.5 126.4 76.5 76.5 197 0 280 0 433.6 26.5 483.5 76.5 560 197 560 280l0 14c0 54.1-43.9 98-98 98-25.9 0-49.5-10.1-67.1-26.5-26.1 37-70.5 54.5-114.9 54.5-40.3 0-75.9-12.9-101.5-38.5S140 320.3 140 280 152.9 204.1 178.5 178.5 239.7 140 280 140 355.9 152.9 381.5 178.5 420 239.7 420 280l0 14c0 23.2 18.8 42 42 42s42-18.8 42-42l0-14c0-71-22.5-126.4-60-164s-93-60-164-60-126.4 22.5-164 60zM364 280c0-28.2-8.8-48.6-22.1-61.9s-33.7-22.1-61.9-22.1-48.6 8.8-61.9 22.1-22.1 33.7-22.1 61.9 8.8 48.6 22.1 61.9 33.7 22.1 61.9 22.1 48.6-8.8 61.9-22.1 22.1-33.7 22.1-61.9z" />
    </svg>
  );
}

// user-police-tie (solid) - 448×512
export function FaUserPoliceTie(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      fill="currentColor"
      {...props}
    >
      <path d="M215.6-29.7L65.7 11.2C55.3 14 48 23.5 48 34.4 48 42.8 52.4 50.7 59.7 55L80 67.2 80 91.8c0 11.4 6 21.8 16 26.7L96 144c0 70.7 57.3 128 128 128s128-57.3 128-128l0-25.5c10-5 16-15.4 16-26.7l0-24.6 20.3-12.2c7.2-4.3 11.7-12.2 11.7-20.6 0-10.8-7.3-20.4-17.7-23.2L232.4-29.7c-5.5-1.5-11.3-1.5-16.8 0zM144 133.7c23.2 5.7 50.9 10.3 80 10.3s56.8-4.6 80-10.3l0 10.3c0 44.2-35.8 80-80 80s-80-35.8-80-80l0-10.3zM254.5 304l-61 0c-9.7 0-17.5 7.8-17.5 17.5 0 4.2 1.5 8.2 4.2 11.4l27.4 32-31 115.1-.6 0-34.6-138.5c-2.2-8.7-11.1-14-19.5-10.8-61.9 23.6-105.9 83.6-105.9 153.8 0 15.1 12.3 27.4 27.4 27.4l361.1 0c15.1 0 27.4-12.3 27.4-27.4 0-70.2-44-130.2-105.9-153.8-8.4-3.2-17.3 2.1-19.5 10.8l-34.6 138.5-.6 0-31-115.1 27.4-32c2.7-3.2 4.2-7.2 4.2-11.4 0-9.7-7.8-17.5-17.5-17.5zM229.7 29.7l28.7 28.7c3.1 3.1 3.1 8.2 0 11.3L229.7 98.3c-3.1 3.1-8.2 3.1-11.3 0L189.7 69.7c-3.1-3.1-3.1-8.2 0-11.3l28.7-28.7c3.1-3.1 8.2-3.1 11.3 0z" />
    </svg>
  );
}

// building-columns (regular) - 512×512
export function FaBuildingColumns(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M269.2 11.9c-8-5.2-18.3-5.2-26.3 0l-232 152C2 169.7-2 180.7 1 190.9S13.4 208 24 208l40 0 0 160-8 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l400 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-8 0 0-160 40 0c10.6 0 20-7 23-17.1s-1-21.1-9.8-26.9l-232-152zM176 368l-64 0 0-160 64 0 0 160zm112 0l-64 0 0-160 64 0 0 160zm112 0l-64 0 0-160 64 0 0 160zM228.3 160L104.4 160 256 60.7 407.6 160 283.7 160c2.7-4.7 4.3-10.2 4.3-16 0-17.7-14.3-32-32-32s-32 14.3-32 32c0 5.8 1.6 11.3 4.3 16zM0 488c0 13.3 10.7 24 24 24l464 0c13.3 0 24-10.7 24-24s-10.7-24-24-24L24 464c-13.3 0-24 10.7-24 24z" />
    </svg>
  );
}

// video (slab-press-regular) - 504×540
export function FaVideo(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 504 540"
      fill="currentColor"
      {...props}
    >
      <path d="M72 90C32.2 90 0 122.2 0 162L0 414c0 39.8 32.2 72 72 72l216 0c39.8 0 72-32.2 72-72l0-24.2 21.1 21.1c20.6 20.6 51.6 26.8 78.5 15.6S504 389.1 504 360l0-144c0-29.1-17.5-55.4-44.4-66.5s-57.9-5-78.5 15.6L360 186.2 360 162c0-39.8-32.2-72-72-72L72 90zM360 252c0-9.5 3.8-18.7 10.5-25.5l36-36c10.3-10.3 25.8-13.4 39.2-7.8S468 201.4 468 216l0 144c0 14.6-8.8 27.7-22.2 33.3s-28.9 2.5-39.2-7.8l-36-36c-6.8-6.8-10.5-15.9-10.5-25.5l0-72zm-36-90l0 252c0 19.9-16.1 36-36 36L72 450c-19.9 0-36-16.1-36-36l0-252c0-19.9 16.1-36 36-36l216 0c19.9 0 36 16.1 36 36z" />
    </svg>
  );
}

// calendar (slab-press-regular) - 432×540
export function FaCalendar(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 432 540"
      fill="currentColor"
      {...props}
    >
      <path d="M234 72c0-9.9-8.1-18-18-18s-18 8.1-18 18l0 18-72 0 0-18c0-9.9-8.1-18-18-18S90 62.1 90 72l0 18-18 0C32.2 90 0 122.2 0 162L0 432c0 39.8 32.2 72 72 72l288 0c39.8 0 72-32.2 72-72l0-270c0-39.8-32.2-72-72-72l-18 0 0-18c0-9.9-8.1-18-18-18s-18 8.1-18 18l0 18-72 0 0-18zM90 126l0 18c0 9.9 8.1 18 18 18s18-8.1 18-18l0-18 72 0 0 18c0 9.9 8.1 18 18 18s18-8.1 18-18l0-18 72 0 0 18c0 9.9 8.1 18 18 18s18-8.1 18-18l0-18 18 0c19.9 0 36 16.1 36 36l0 54-360 0 0-54c0-19.9 16.1-36 36-36l18 0zM396 432c0 19.9-16.1 36-36 36L72 468c-19.9 0-36-16.1-36-36l0-180 360 0 0 180z" />
    </svg>
  );
}

// =============================================================================
// LOCATION ICONS
// =============================================================================

// location-dot (jelly-regular) - 448×560
export function FaLocationDot(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 560"
      fill="currentColor"
      {...props}
    >
      <path d="M224 126c-27.4 0-52.5 8.8-70.8 27.2s-27.2 43.4-27.2 70.8 8.8 52.5 27.2 70.8 43.4 27.2 70.8 27.2 52.5-8.8 70.8-27.2 27.2-43.4 27.2-70.8-8.8-52.5-27.2-70.8-43.4-27.2-70.8-27.2zm-42 98c0-15.4 4.8-25.3 10.8-31.2s15.9-10.8 31.2-10.8 25.3 4.8 31.2 10.8 10.8 15.9 10.8 31.2-4.8 25.3-10.8 31.2-15.9 10.8-31.2 10.8-25.3-4.8-31.2-10.8-10.8-15.9-10.8-31.2zM224 0C158.1 0 101.5 21.1 61.3 61.3S0 158.1 0 224c0 72.8 28.3 139.3 61.1 192.1 32.9 53 71.4 94.3 94.8 117 18.8 18.3 43.6 26.9 68.1 26.9s49.4-8.6 68.1-26.9c23.4-22.8 61.9-64 94.8-117 32.7-52.8 61.1-119.4 61.1-192.1 0-65.9-21.1-122.5-61.3-162.7S289.9 0 224 0zM56 224c0-53.9 17-95.3 44.9-123.1S170.1 56 224 56 319.3 73 347.1 100.9 392 170.1 392 224c0 58.5-22.9 114.6-52.7 162.6-29.6 47.8-64.8 85.5-86.2 106.4-7.1 6.9-17.4 11-29.1 11s-21.9-4.1-29.1-11c-21.5-20.9-56.6-58.6-86.2-106.4-29.7-48-52.7-104.1-52.7-162.6z" />
    </svg>
  );
}

// building (regular) - 384×512
export function FaBuilding(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 384 512"
      fill="currentColor"
      {...props}
    >
      <path d="M64 48c-8.8 0-16 7.2-16 16l0 384c0 8.8 7.2 16 16 16l80 0 0-80c0-17.7 14.3-32 32-32l32 0c17.7 0 32 14.3 32 32l0 80 80 0c8.8 0 16-7.2 16-16l0-384c0-8.8-7.2-16-16-16L64 48zM0 64C0 28.7 28.7 0 64 0L320 0c35.3 0 64 28.7 64 64l0 384c0 35.3-28.7 64-64 64L64 512c-35.3 0-64-28.7-64-64L0 64zm96 48c0-8.8 7.2-16 16-16l32 0c8.8 0 16 7.2 16 16l0 32c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-32zM240 96l32 0c8.8 0 16 7.2 16 16l0 32c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-32c0-8.8 7.2-16 16-16zM96 240c0-8.8 7.2-16 16-16l32 0c8.8 0 16 7.2 16 16l0 32c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-32zm144-16l32 0c8.8 0 16 7.2 16 16l0 32c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-32c0-8.8 7.2-16 16-16z" />
    </svg>
  );
}

// =============================================================================
// ALERT ICONS
// =============================================================================

// circle-exclamation (utility-semibold) - 512×512
export function FaCircleExclamation(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M256 512a256 256 0 1 1 0-512 256 256 0 1 1 0 512zm0-448a192 192 0 1 0 0 384 192 192 0 1 0 0-384zm0 328a40 40 0 1 1 0-80 40 40 0 1 1 0 80zm28-120l-56 0 0-144 56 0 0 144z" />
    </svg>
  );
}

// =============================================================================
// COPY & CLIPBOARD ICONS
// =============================================================================

// copy (regular) - 448×512
export function FaCopy(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      fill="currentColor"
      {...props}
    >
      <path d="M384 336l-192 0c-8.8 0-16-7.2-16-16l0-256c0-8.8 7.2-16 16-16l133.5 0c4.2 0 8.3 1.7 11.3 4.7l58.5 58.5c3 3 4.7 7.1 4.7 11.3L400 320c0 8.8-7.2 16-16 16zM192 384l192 0c35.3 0 64-28.7 64-64l0-197.5c0-17-6.7-33.3-18.7-45.3L370.7 18.7C358.7 6.7 342.5 0 325.5 0L192 0c-35.3 0-64 28.7-64 64l0 256c0 35.3 28.7 64 64 64zM64 128c-35.3 0-64 28.7-64 64L0 448c0 35.3 28.7 64 64 64l192 0c35.3 0 64-28.7 64-64l0-16-48 0 0 16c0 8.8-7.2 16-16 16L64 464c-8.8 0-16-7.2-16-16l0-256c0-8.8 7.2-16 16-16l16 0 0-48-16 0z" />
    </svg>
  );
}

// clipboard-check (regular) - 384×512
export function FaClipboardCheck(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 384 512"
      fill="currentColor"
      {...props}
    >
      <path d="M232 0c25 0 47 12.7 59.9 32L320 32c35.3 0 64 28.7 64 64l0 352c0 35.3-28.7 64-64 64L64 512c-35.3 0-64-28.7-64-64L0 96C0 60.7 28.7 32 64 32l28.1 0C105 12.7 127 0 152 0l80 0zM64 80c-8.8 0-16 7.2-16 16l0 352c0 8.8 7.2 16 16 16l256 0c8.8 0 16-7.2 16-16l0-352c0-8.8-7.2-16-16-16l-16.4 0c-4 36-34.5 64-71.6 64l-80 0c-37.1 0-67.6-28-71.6-64L64 80zM228.7 239.9c7.8-10.7 22.8-13.1 33.5-5.3s13.1 22.8 5.3 33.5L191.4 372.8c-4.2 5.7-10.7 9.3-17.8 9.8-7.1 .5-14-2.2-18.9-7.3l-35.9-37.2c-9.2-9.5-8.9-24.7 .6-33.9 9.5-9.2 24.7-9 33.9 .6l16.1 16.6 59.3-81.6zM152 48c-13.3 0-24 10.7-24 24s10.7 24 24 24l80 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-80 0z" />
    </svg>
  );
}

// =============================================================================
// CHECK / SUCCESS ICONS
// =============================================================================

// circle-check (jelly-regular) - 560×560
export function FaCheck(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path d="M409.5 217.9c12.1-9.7 14-27.3 4.4-39.4s-27.3-14-39.4-4.4c-52.5 42-100.2 111.8-123.8 149.3-15.3-20.1-39.5-48.3-67.2-66.8-12.9-8.6-30.3-5.1-38.8 7.8s-5.1 30.3 7.8 38.8c28.9 19.3 55.6 55.9 65.6 70.7 7.4 11 19.9 18 33.8 18 14.4 0 27.6-7.6 34.9-19.9 14.4-24.5 67.2-109.7 122.8-154.2zM280 0C197 0 126.4 26.5 76.5 76.5S0 197 0 280 26.5 433.6 76.5 483.5 197 560 280 560 433.6 533.5 483.5 483.5 560 363 560 280 533.5 126.4 483.5 76.5 363 0 280 0zM56 280c0-71 22.5-126.4 60-164s93-60 164-60 126.4 22.5 164 60 60 93 60 164-22.5 126.4-60 164-93 60-164 60-126.4-22.5-164-60-60-93-60-164z" />
    </svg>
  );
}

// Alias for backward compatibility
export const FaCircleCheck = FaCheck;

// =============================================================================
// VISIBILITY ICONS
// =============================================================================

// eye (jelly-regular) - 616×560
export function FaEye(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 616 560"
      fill="currentColor"
      {...props}
    >
      <path d="M308 168c-14.6 0-26.1 10.3-29 23.3-4.2 18.7-21 32.7-41 32.7-14 0-33.6 8.1-38.8 29.5-2.1 8.5-3.2 17.4-3.2 26.5 0 61.9 50.1 112 112 112s112-50.1 112-112-50.1-112-112-112zM252 279c32-4.6 59-24.6 73.2-52.3 22.5 7.3 38.8 28.4 38.8 53.3 0 30.9-25.1 56-56 56-30.9 0-56.5-26.1-56-57zM308 42C138.8 42 46.6 165.5 15.9 216 4.2 235.2 0 256.6 0 276.6l0 6.8c0 20 4.2 41.4 15.9 60.6 30.7 50.5 122.9 174 292.1 174S569.4 394.5 600.1 344c11.7-19.2 15.9-40.6 15.9-60.6l0-6.8c0-20-4.2-41.4-15.9-60.6-30.7-50.5-122.9-174-292.1-174zM63.8 245.1C91.6 199.2 169.2 98 308 98S524.4 199.2 552.2 245.1c5.1 8.5 7.8 19.1 7.8 31.5l0 6.8c0 12.4-2.6 23.1-7.8 31.5-27.9 45.8-105.4 147.1-244.2 147.1S91.6 360.8 63.8 314.9c-5.1-8.5-7.8-19.1-7.8-31.5l0-6.8c0-12.4 2.6-23.1 7.8-31.5z" />
    </svg>
  );
}

// eye-slash (jelly-regular) - 616×560
export function FaEyeSlash(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 616 560"
      fill="currentColor"
      {...props}
    >
      <path d="M75.8 8.2C64.9-2.7 47.1-2.7 36.2 8.2s-10.9 28.7 0 39.6c168 168 336 336 504 504 10.9 10.9 28.7 10.9 39.6 0s10.9-28.7 0-39.6l-67.4-67.4c44.1-35.8 72.7-76.2 87.6-100.8 11.7-19.2 15.9-40.6 15.9-60.6l0-6.8c0-20-4.2-41.4-15.9-60.6-30.7-50.5-122.9-174-292.1-174-62.6 0-114.9 17-157.4 41L75.8 8.2zM192.1 124.5c32.4-15.9 70.8-26.5 115.9-26.5 138.8 0 216.4 101.2 244.2 147.1 5.1 8.5 7.8 19.1 7.8 31.5l0 6.8c0 12.4-2.6 23.1-7.8 31.5-13.5 22.2-39.7 58.9-79.7 90L192.1 124.5zM94.8 202c10-11.8 8.5-29.5-3.3-39.5S62 154.1 52 165.9C35.9 185 24 202.7 15.9 216 4.2 235.2 0 256.6 0 276.6l0 6.8c0 20 4.2 41.4 15.9 60.6 30.7 50.5 122.9 174 292.1 174 25.1 0 48.6-2.7 70.5-7.6 15.1-3.3 24.6-18.3 21.3-33.4s-18.3-24.6-33.4-21.3c-18 4-37.4 6.3-58.4 6.3-138.8 0-216.4-101.2-244.2-147.1-5.1-8.5-7.8-19.1-7.8-31.5l0-6.8c0-12.4 2.6-23.1 7.8-31.5 6.9-11.3 17.2-26.6 31-43z" />
    </svg>
  );
}

// =============================================================================
// PHONE ICONS
// =============================================================================

// phone (slab-press-regular) - 468×540
export function FaPhone(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 468 540"
      fill="currentColor"
      {...props}
    >
      <path d="M86.1 66.1c28.6-19 66.6-15.3 90.8 9l45 45c28.1 28.1 28.1 73.7 0 101.8l-48.7 48.7c18.8 32.3 45.8 59.3 78.2 78.2l48.7-48.7c28.1-28.1 73.7-28.1 101.8 0l45 45c24.3 24.3 28 62.3 9 90.9l-35.5 53.2c-3.7 5.7-8.1 10.8-13.2 15.2-12.6 11-29.2 17.7-47.3 17.7-198.8 0-360-161.2-360-360 0-25.4 13.1-47.6 32.9-60.4L86.1 66.1zm65.4 34.5c-12.1-12.1-31.1-14-45.4-4.5L52.5 131.7c-10 6.4-16.5 17.6-16.5 30.3 0 178.9 145.1 324 324 324 12.3 0 23.8-6.5 30.4-16.7L426 416c9.5-14.3 7.6-33.3-4.5-45.4l-45-45c-14.1-14.1-36.9-14.1-50.9 0l-58.2 58.2c-5.5 5.5-13.9 6.8-20.8 3.3-48.1-24.3-87.3-63.5-111.6-111.6-3.5-6.9-2.2-15.3 3.3-20.8l58.2-58.2c14.1-14.1 14.1-36.9 0-50.9l-45-45z" />
    </svg>
  );
}

// Alias - use single phone style everywhere
export const FaPhoneSolid = FaPhone;
export const FaTelephone = FaPhone;
export const FaTelephoneOutbound = FaPhone;

// =============================================================================
// LOADING / SPINNER ICONS
// =============================================================================

// spinner-scale (solid) - 448×512
export function FaSpinnerScale(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      fill="currentColor"
      {...props}
    >
      <path d="M224 116a52 52 0 1 1 0-104 52 52 0 1 1 0 104zm0 300c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32zM416 288a32 32 0 1 1 0-64 32 32 0 1 1 0 64zM0 256a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zm399.4-96.2a56 56 0 1 1 -79.2-79.2 56 56 0 1 1 79.2 79.2zM65.6 414.4a32 32 0 1 1 45.3-45.3 32 32 0 1 1 -45.3 45.3zm271.5 0a32 32 0 1 1 45.3-45.3 32 32 0 1 1 -45.3 45.3zM54.3 86.3a48 48 0 1 1 67.9 67.9 48 48 0 1 1 -67.9-67.9z" />
    </svg>
  );
}

// Alias for backward compatibility
export const FaArrowsRotate = FaSpinnerScale;

// =============================================================================
// MISC ICONS
// =============================================================================

// hashtag (regular) - 512×512
export function FaHashtag(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M226.9 29c2.8-13-5.5-25.7-18.4-28.5S182.7 6 180 18.9L156.6 128 68.5 128c-13.3 0-24 10.7-24 24s10.7 24 24 24l77.8 0-34.3 160-88 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l77.7 0-21.2 99c-2.8 13 5.5 25.7 18.4 28.5s25.7-5.5 28.5-18.4l23.4-109 155.4 0-21.2 99c-2.8 13 5.5 25.7 18.4 28.5s25.7-5.5 28.5-18.4l23.4-109 88 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-77.7 0 34.3-160 88.1 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-77.8 0 21.2-99c2.8-13-5.5-25.7-18.4-28.5S387.2 6 384.4 18.9l-23.4 109.1-155.4 0 21.2-99zM195.4 176l155.4 0-34.3 160-155.4 0 34.3-160z" />
    </svg>
  );
}

// =============================================================================
// BRAND ICONS
// =============================================================================

// scale-balanced (regular) - 640×512 - for courts
export function FaScaleBalanced(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 640 512"
      fill="currentColor"
      {...props}
    >
      <path d="M520 48L393.3 48C381 19.7 352.8 0 320 0s-61 19.7-73.3 48L120 48c-13.3 0-24 10.7-24 24s10.7 24 24 24l121.6 0c5.8 28.6 26.9 51.7 54.4 60.3l0 307.7-176 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l400 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-176 0 0-307.7c27.5-8.6 48.6-31.7 54.4-60.3L520 96c13.3 0 24-10.7 24-24s-10.7-24-24-24zm-8 147.8L584.4 320 439.6 320 512 195.8zM386 337.1C396.8 382 449.1 416 512 416s115.2-34 126-78.9c2.6-11-1-22.3-6.7-32.1L536.1 141.8c-5-8.6-14.2-13.8-24.1-13.8s-19.1 5.3-24.1 13.8L392.7 305.1c-5.7 9.8-9.3 21.1-6.7 32.1zM54.4 320l72.4-124.2 72.4 124.2-144.9 0zm72.4 96c62.9 0 115.2-34 126-78.9 2.6-11-1-22.3-6.7-32.1L150.9 141.8c-5-8.6-14.2-13.8-24.1-13.8s-19.1 5.3-24.1 13.8L7.6 305.1C1.9 314.8-1.8 326.1 .9 337.1 11.7 382 64 416 126.8 416zM320 48a32 32 0 1 1 0 64 32 32 0 1 1 0-64z" />
    </svg>
  );
}

// building-shield (regular) - 576×512 - for corrections centres
export function FaBuildingShield(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 576 512"
      fill="currentColor"
      {...props}
    >
      <path d="M96 48l256 0c8.8 0 16 7.2 16 16l0 161 38.7-12.9c3.1-1 6.2-1.8 9.3-2.5L416 64c0-35.3-28.7-64-64-64L96 0C60.7 0 32 28.7 32 64l0 384c0 35.3 28.7 64 64 64l201.2 0C261 469.4 240 414.5 240 356.4l0-4.4-32 0c-17.7 0-32 14.3-32 32l0 80-80 0c-8.8 0-16-7.2-16-16L80 64c0-8.8 7.2-16 16-16zM320 240c0-8.8-7.2-16-16-16l-32 0c-8.8 0-16 7.2-16 16l0 32c0 1.6 .2 3.1 .6 4.5 9.5-12.3 22.6-21.9 38.1-27.1l25.3-8.4 0-1zM128 112l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0c-8.8 0-16 7.2-16 16zM272 96c-8.8 0-16 7.2-16 16l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0zM128 240l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0c-8.8 0-16 7.2-16 16zM445.3 488.5l-13.3 6.3 0-188.1 96 32 0 19.6c0 55.8-32.2 106.5-82.7 130.3zM421.9 259.5l-112 37.3c-13.1 4.4-21.9 16.6-21.9 30.4l0 31.1c0 74.4 43 142.1 110.2 173.7l18.5 8.7c4.8 2.2 10 3.4 15.2 3.4s10.5-1.2 15.2-3.4l18.5-8.7C533 500.3 576 432.6 576 358.2l0-31.1c0-13.8-8.8-26-21.9-30.4l-112-37.3c-6.6-2.2-13.7-2.2-20.2 0z" />
    </svg>
  );
}

// gavel (solid) - 512×512 - for bail hubs
export function FaGavel(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M318.6 9.4c-12.5-12.5-32.8-12.5-45.3 0l-120 120c-12.5 12.5-12.5 32.8 0 45.3l16 16c12.5 12.5 32.8 12.5 45.3 0l4-4L325.4 293.4l-4 4c-12.5 12.5-12.5 32.8 0 45.3l16 16c12.5 12.5 32.8 12.5 45.3 0l120-120c12.5-12.5 12.5-32.8 0-45.3l-16-16c-12.5-12.5-32.8-12.5-45.3 0l-4 4L330.6 74.6l4-4c12.5-12.5 12.5-32.8 0-45.3l-16-16zm-152 288c-12.5-12.5-32.8-12.5-45.3 0l-112 112c-12.5 12.5-12.5 32.8 0 45.3l48 48c12.5 12.5 32.8 12.5 45.3 0l112-112c12.5-12.5 12.5-32.8 0-45.3l-48-48z" />
    </svg>
  );
}

// Microsoft Teams (Bootstrap Icons) - 16×16
export function FaMicrosoftTeams(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 16 16"
      fill="currentColor"
      {...props}
    >
      <path d="M9.186 4.797a2.42 2.42 0 1 0-2.86-2.448h1.178c.929 0 1.682.753 1.682 1.682zm-4.295 7.738h2.613c.929 0 1.682-.753 1.682-1.682V5.58h2.783a.7.7 0 0 1 .682.716v4.294a4.197 4.197 0 0 1-4.093 4.293c-1.618-.04-3-.99-3.667-2.35Zm10.737-9.372a1.674 1.674 0 1 1-3.349 0 1.674 1.674 0 0 1 3.349 0m-2.238 9.488-.12-.002a5.2 5.2 0 0 0 .381-2.07V6.306a1.7 1.7 0 0 0-.15-.725h1.792c.39 0 .707.317.707.707v3.765a2.6 2.6 0 0 1-2.598 2.598z" />
      <path d="M.682 3.349h6.822c.377 0 .682.305.682.682v6.822a.68.68 0 0 1-.682.682H.682A.68.68 0 0 1 0 10.853V4.03c0-.377.305-.682.682-.682Zm5.206 2.596v-.72h-3.59v.72h1.357V9.66h.87V5.945z" />
    </svg>
  );
}

// Clock (FA Sharp Solid) - 512×512
export function FaClock(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M256 0a256 256 0 1 1 0 512 256 256 0 1 1 0-512zM232 120l0 148.8 10.7 7.1 96 64 20 13.3 26.6-39.9c-2.4-1.6-37.5-25-105.3-70.2l0-147.2-48 0 0 24z" />
    </svg>
  );
}

// Users (FA Sharp Solid) - 640×512
export function FaUsers(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 640 512"
      fill="currentColor"
      {...props}
    >
      <path d="M320 16a104 104 0 1 1 0 208 104 104 0 1 1 0-208zM96 88a72 72 0 1 1 0 144 72 72 0 1 1 0-144zm14.7 392l-110.7 0 48-192 107 0-41.8 181.2-2.5 10.8zm418.5 0l-2.5-10.8-41.8-181.2 107 0 48 192-110.7 0zM472 160a72 72 0 1 1 144 0 72 72 0 1 1 -144 0zM208 272l224 0 48 208-320 0 48-208z" />
    </svg>
  );
}

// Folder (FA Sharp Solid) - 512×512
export function FaFolder(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M0 448l512 0 0-368-224 0-64-48-224 0 0 416z" />
    </svg>
  );
}

// Star Jelly (FA Jelly Regular) - 616×560
export function FaStarJelly(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 616 560"
      fill="currentColor"
      {...props}
    >
      <path d="M252.1 4.1C263.9-16.6 285.7-28 308.2-28s44.3 11.4 56.1 32.1c13.5 23.6 38.3 68.8 56.8 122.3 56.7 1.1 105.6 9.5 132.2 15 23.3 4.9 40.9 22.1 47.8 43.4s2.8 45.6-13.2 63.3c-18.3 20.1-52.9 55.7-98.1 89.9 16.5 54.2 23 105.3 25.9 132.3 2.6 23.7-8.4 45.7-26.5 58.9s-42.5 16.8-64.2 7c-24.8-11.2-70.3-33.1-116.8-65.6-46.5 32.4-92 54.4-116.8 65.6-21.7 9.8-46.1 6.2-64.2-7S98 493.9 100.6 470.3c3-27 9.5-78.1 25.9-132.3-45.2-34.2-79.9-69.7-98.1-89.9-16-17.6-20.1-41.9-13.2-63.3s24.5-38.6 47.8-43.4c26.6-5.5 75.5-13.9 132.2-15 18.5-53.5 43.3-98.7 56.8-122.3zm48.6 27.8c-12.4 21.7-33.8 62.4-50.8 111.6-7.9 22.9-29.3 38.4-53.5 38.9-52.1 1-97.3 8.7-121.8 13.9-5.9 1.2-8.6 9.8-4.6 14.2 16.8 18.5 48.9 51.4 90.4 82.8 19.3 14.6 27.5 39.8 20.4 62.9-15.1 49.8-21.8 95.3-24.5 120.2-.7 6 6.6 11.2 12.1 8.8 22.8-10.3 64-30.6 106.7-60.4 19.9-13.9 46.3-13.9 66.2 0 42.7 29.8 83.9 50.1 106.7 60.4 5.5 2.5 12.7-2.8 12.1-8.8-2.7-24.9-9.3-70.4-24.5-120.2-7-23.2 1.1-48.3 20.4-62.9 41.5-31.4 73.6-64.3 90.4-82.8 4-4.4 1.2-13-4.6-14.2-24.5-5.1-69.8-12.9-121.9-13.9-24.2-.5-45.6-16-53.5-38.9-17-49.2-38.4-89.9-50.8-111.6-3-5.2-12-5.2-14.9 0z" />
    </svg>
  );
}

// Star Jelly Fill (FA Jelly Fill Regular) - 616×560
export function FaStarJellyFill(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 616 560"
      fill="currentColor"
      {...props}
    >
      <path d="M308.2-28c-22.4 0-44.3 11.4-56.1 32.1-13.5 23.6-36.6 67.5-55.1 121.1-.2 .7-.9 1.2-1.7 1.2-56.7 1.1-105.6 9.5-132.2 15-23.3 4.9-40.9 22.1-47.8 43.4s-2.8 45.6 13.2 63.3c18.3 20.1 52.9 55.7 98.1 89.9 .6 .4 .8 1.3 .6 2-16.5 54.2-23.6 103.3-26.6 130.4-2.6 23.7 8.4 45.7 26.5 58.9s42.5 16.8 64.2 7c24.8-11.2 69.3-33.1 115.8-65.6 .6-.4 1.5-.4 2.1 0 46.5 32.4 91 54.4 115.8 65.6 21.7 9.8 46.1 6.2 64.2-7s29.1-35.2 26.5-58.9c-3-27-10.1-76.2-26.6-130.4-.2-.7 .1-1.5 .6-2 45.2-34.2 79.9-69.7 98.1-89.9 16-17.6 20.1-41.9 13.2-63.3s-24.5-38.6-47.8-43.4c-26.6-5.5-75.5-13.9-132.2-15-.7 0-1.4-.5-1.7-1.2-18.5-53.5-41.6-97.5-55.1-121.1-11.8-20.7-33.6-32.1-56.1-32.1z" />
    </svg>
  );
}

// Fax (FA Regular) - 512×512
export function FaFax(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path d="M160 64c0-8.8 7.2-16 16-16l159.4 0c4.2 0 8.3 1.7 11.3 4.7l48.6 48.6c3 3 4.7 7.1 4.7 11.3l0 31.4 48 0 0-31.4c0-17-6.7-33.3-18.7-45.3L380.7 18.7C368.7 6.7 352.4 0 335.4 0L176 0c-35.3 0-64 28.7-64 64l0 64-72 0c-22.1 0-40 17.9-40 40L0 440c0 22.1 17.9 40 40 40l432 0c22.1 0 40-17.9 40-40l0-208c0-22.1-17.9-40-40-40l-312 0 0-128zM112 176l0 256-64 0 0-256 64 0zm48 256l0-192 304 0 0 192-304 0zm72-120a24 24 0 1 0 0-48 24 24 0 1 0 0 48zm24 72a24 24 0 1 0 -48 0 24 24 0 1 0 48 0zm64-72a24 24 0 1 0 0-48 24 24 0 1 0 0 48zm24 72a24 24 0 1 0 -48 0 24 24 0 1 0 48 0zm64-72a24 24 0 1 0 0-48 24 24 0 1 0 0 48zm24 72a24 24 0 1 0 -48 0 24 24 0 1 0 48 0z" />
    </svg>
  );
}

// Pen Line (FA Utility Semibold) - 576×512
export function FaPenLine(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 576 512"
      fill="currentColor"
      {...props}
    >
      <path d="M389.5 0c67.7 0 122.5 54.8 122.5 122.5 0 32.5-12.9 63.7-35.9 86.6L173.2 512 0 512 0 338.7 302.9 35.9C325.8 12.9 357 0 389.5 0zM576 512l-288 0 0-64 288 0 0 64zM389.5 64c-15.5 0-30.4 6.2-41.4 17.1L64 365.3 64 448 146.8 448 430.9 163.9c11-11 17.1-25.9 17.1-41.4 0-32.3-26.2-58.5-58.5-58.5z" />
    </svg>
  );
}

// =============================================================================
// COMMUNICATION ICONS (Jelly Duo)
// =============================================================================

// Extended props for duotone icons
interface DuotoneIconProps extends IconProps {
  secondaryOpacity?: number;
  primaryOpacity?: number;
}

// Comment Dots (FA Jelly Duo Regular) - 560×560
export function FaCommentDots({
  secondaryOpacity = 0.4,
  primaryOpacity = 1,
  ...props
}: DuotoneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path
        opacity={secondaryOpacity}
        d="M56 238c0 39.9 8 72.8 23.2 99 15.1 25.8 38.3 46.7 72 61.1 11.6 5 22.1 13.8 28.4 25.8 6.6 12.6 8.2 28.8 .2 44.2-2.1 3.9-4.2 7.4-6.3 10.6-4.2 6.4-5.5 11-5.5 13.8 0 6.4 5.2 11.6 11.6 11.6l1.5 0c2.4-.2 4.6-.7 6.6-1.8 .6-.4 1.2-.8 1.8-1.2l82.3-68.6c9.7-8.1 21.7-12.4 33.6-13 74.4-3.8 123.2-24.3 153.3-54 29.9-29.5 45.3-71.7 45.3-127.3 0-58.6-17.1-102.5-50.5-132.2-33.9-30.2-89.2-49.8-173.5-49.8S140.4 75.6 106.5 105.8C73.1 135.5 56 179.4 56 238zm154 0a28 28 0 1 1 -56 0 28 28 0 1 1 56 0zm98 0a28 28 0 1 1 -56 0 28 28 0 1 1 56 0zm98 0a28 28 0 1 1 -56 0 28 28 0 1 1 56 0z"
      />
      <path
        opacity={primaryOpacity}
        d="M280 266a28 28 0 1 0 0-56 28 28 0 1 0 0 56zm70-28a28 28 0 1 1 56 0 28 28 0 1 1 -56 0zM182 266a28 28 0 1 0 0-56 28 28 0 1 0 0 56zM280 0c-92 0-162.8 21.3-210.8 64-48.5 43.2-69.2 104.3-69.2 174 0 47.2 9.4 90.5 30.9 127.2 21.2 36.2 53.1 64.3 95.5 83.1-7.2 11.1-14.4 26.3-14.4 44.1 0 37.3 30.3 67.6 67.6 67.6 16.7 0 32.8-5.2 45.8-16l82.9-68.7c82.8-4.2 146.7-27.5 189.8-70.1 43.4-42.8 62-101.2 62-167.2 0-69.7-20.7-130.8-69.2-174-48-42.7-118.7-64-210.8-64zM56 238C56 179.4 73.1 135.5 106.5 105.8 140.4 75.6 195.6 56 280 56s139.6 19.6 173.5 49.8c33.4 29.7 50.5 73.6 50.5 132.2 0 55.6-15.4 97.9-45.3 127.3-30.1 29.7-78.9 50.2-153.3 54-11.9 .6-23.9 4.9-33.6 13L189.5 501c-2.9 2.4-6.3 3-9.9 3-6.4 0-11.6-5.2-11.6-11.6 0-2.9 1.3-7.4 5.5-13.8 2.1-3.1 4.2-6.7 6.3-10.6 8.1-15.4 6.4-31.6-.2-44.2-6.3-11.9-16.8-20.8-28.4-25.8-33.7-14.4-56.9-35.3-72-61.1-15.3-26.1-23.2-59.1-23.2-99z"
      />
    </svg>
  );
}

// =============================================================================
// SHERIFF CELLS ICONS (Duotone)
// =============================================================================

// Badge Sheriff (Sharp Duotone Light) - 512×512 - for Courthouse Cells
export function FaBadgeSheriff({
  secondaryOpacity = 0.4,
  primaryOpacity = 1,
  ...props
}: DuotoneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path
        opacity={secondaryOpacity}
        d="M84.6 167.7c35.4 56.6 53.8 86.1 55.2 88.3-1.4 2.2-19.8 31.7-55.2 88.3 2 2.3 3.9 4.9 5.5 7.7 1.7 3 3.1 6 4.1 9.1l103.8 0c1 1.7 17.4 30.9 49.3 87.7 2.9-.6 6-.9 9.1-.9s6.1 .3 9.1 .9c31.9-56.8 48.4-86 49.3-87.7l103.8 0c1-3.1 2.4-6.2 4.1-9.1 1.6-2.8 3.4-5.3 5.5-7.7-35.4-56.6-53.8-86.1-55.2-88.3 1.4-2.2 19.8-31.7 55.2-88.3-2-2.3-3.9-4.9-5.5-7.7-1.7-3-3.1-6-4.1-9.1l-103.8 0c-1-1.7-17.4-30.9-49.3-87.7-2.9 .6-6 .9-9.1 .9s-6.1-.3-9.1-.9c-31.9 56.8-48.4 86-49.3 87.7l-103.8 0c-1 3.1-2.4 6.2-4.1 9.1-1.6 2.8-3.4 5.3-5.5 7.7zM161.7 256l11.3-11.3 72-72 11.3-11.3 11.3 11.3 72 72 11.3 11.3-11.3 11.3-72 72-11.3 11.3-11.3-11.3-72-72-11.3-11.3z"
      />
      <path
        opacity={primaryOpacity}
        d="M256.4 32a16 16 0 1 0 0-32 16 16 0 1 0 0 32zm0 32c-3.1 0-6.1-.3-9.1-.9-31.9 56.8-48.4 86-49.3 87.7l-103.8 0c-1 3.1-2.4 6.2-4.1 9.1-1.6 2.8-3.4 5.3-5.5 7.7 35.4 56.6 53.8 86.1 55.2 88.3-1.4 2.2-19.8 31.7-55.2 88.3 2 2.3 3.9 4.9 5.5 7.7 1.7 3 3.1 6 4.1 9.1l103.8 0c1 1.7 17.4 30.9 49.3 87.7 2.9-.6 6-.9 9.1-.9s6.1 .3 9.1 .9c31.9-56.8 48.4-86 49.3-87.7l103.8 0c1-3.1 2.4-6.2 4.1-9.1 1.6-2.8 3.4-5.3 5.5-7.7-35.4-56.6-53.8-86.1-55.2-88.3 1.4-2.2 19.8-31.7 55.2-88.3-2-2.3-3.9-4.9-5.5-7.7-1.7-3-3.1-6-4.1-9.1l-103.8 0c-1-1.7-17.4-30.9-49.3-87.7-2.9 .6-6 .9-9.1 .9zm-163 54.9l85.9 0 40.5-71.9c-7.1-8.4-11.3-19.2-11.3-30.9 0-26.5 21.5-48 48-48s48 21.5 48 48c0 11.8-4.3 22.6-11.3 30.9l40.4 71.9 85.9 0c3.8-9.9 10.9-18.7 20.8-24.4 23-13.3 52.3-5.4 65.6 17.6s5.4 52.3-17.6 65.6c-10.1 5.8-21.5 7.6-32.1 5.7l-45.4 72.7 45.4 72.7c10.7-1.8 22-.1 32.1 5.7 23 13.3 30.8 42.6 17.6 65.6s-42.6 30.8-65.6 17.6c-9.9-5.7-17-14.5-20.8-24.4l-85.9 0-40.5 71.9c7.1 8.4 11.3 19.2 11.3 30.9 0 26.5-21.5 48-48 48s-48-21.5-48-48c0-11.8 4.3-22.6 11.3-30.9l-40.4-71.9-85.9 0c-3.8 9.9-10.9 18.7-20.8 24.4-23 13.3-52.3 5.4-65.6-17.6s-5.4-52.3 17.6-65.6c10.1-5.8 21.5-7.6 32.1-5.7l45.4-72.7-45.4-72.7c-10.7 1.8-22 .1-32.1-5.7-23-13.3-30.8-42.6-17.6-65.6S49.6 81.2 72.5 94.4c9.9 5.7 17 14.5 20.8 24.4zM339.7 267.3l-72 72-11.3 11.3-11.3-11.3-72-72-11.3-11.3 11.3-11.3 72-72 11.3-11.3 11.3 11.3 72 72 11.3 11.3-11.3 11.3zM207 256l49.4 49.4 49.4-49.4-49.4-49.4-49.4 49.4zM472.2 149.9a16 16 0 1 0 -16-27.7 16 16 0 1 0 16 27.7zM450.4 368a16 16 0 1 0 27.7 16 16 16 0 1 0 -27.7-16zm-178 128a16 16 0 1 0 -32 0 16 16 0 1 0 32 0zM34.7 384a16 16 0 1 0 27.7-16 16 16 0 1 0 -27.7 16zm5.9-234.1a16 16 0 1 0 16-27.7 16 16 0 1 0 -16 27.7z"
      />
    </svg>
  );
}

// Dungeon (Duotone Light) - 512×512 - for PD/RCMP Cells
export function FaDungeon({
  secondaryOpacity = 0.4,
  primaryOpacity = 1,
  ...props
}: DuotoneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      {...props}
    >
      <path
        opacity={secondaryOpacity}
        d="M32 256l64 0c0-20 3.7-39.2 10.4-56.9L49 170.4C38 196.7 32 225.7 32 256zm0 32l0 80 64 0 0-80-64 0zm0 112l0 64c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-64-64 0zM63.3 141.8l57.5 28.7c16.2-25.5 39.3-46.1 66.8-59.1l-39-52c-35.3 19.3-64.8 47.8-85.3 82.3zM178.4 45.8l40.8 54.4C231 97.5 243.3 96 256 96s25 1.5 36.8 4.3l40.8-54.4C309.4 36.9 283.3 32 256 32s-53.4 4.9-77.6 13.8zm146.1 65.5c27.1 12.8 50 33.1 66.1 58l54.9-32.9c-20.3-32.1-48.6-58.7-82-77.1l-39 52zM405.2 198c7 18 10.8 37.6 10.8 58l64 0c0-32.5-6.9-63.4-19.4-91.3L405.2 198zM416 288l0 80 64 0 0-80-64 0zm0 112l0 64c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-64-64 0z"
      />
      <path
        opacity={primaryOpacity}
        d="M219.2 100.3L178.4 45.8C202.6 36.9 228.7 32 256 32s53.4 4.9 77.6 13.8l-40.8 54.4C281 97.5 268.7 96 256 96s-25 1.5-36.8 4.3zm-31.7 11.1c-27.5 13-50.6 33.6-66.8 59.1L63.3 141.8c20.5-34.5 50-63 85.3-82.3l39 52zm-81.1 87.7C99.7 216.8 96 236 96 256l-64 0c0-30.3 6-59.3 17-85.6l57.5 28.7zM96 288l0 80-64 0 0-80 64 0zm0 112l0 64c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-64 64 0zM363.4 59.4c33.4 18.3 61.7 44.9 82 77.1l-54.9 32.9c-16.1-25-39-45.2-66.1-58l39-52zm97.2 105.3C473.1 192.6 480 223.5 480 256l-64 0c0-20.5-3.8-40-10.8-58l55.5-33.3zM480 288l0 80-64 0 0-80 64 0zm0 112l0 64c0 8.8-7.2 16-16 16l-32 0c-8.8 0-16-7.2-16-16l0-64 64 0zM128 256c0-70.7 57.3-128 128-128s128 57.3 128 128l0 208c0 26.5 21.5 48 48 48l32 0c26.5 0 48-21.5 48-48l0-208C512 114.6 397.4 0 256 0S0 114.6 0 256L0 464c0 26.5 21.5 48 48 48l32 0c26.5 0 48-21.5 48-48l0-208zm128-80c-8.8 0-16 7.2-16 16l0 304c0 8.8 7.2 16 16 16s16-7.2 16-16l0-304c0-8.8-7.2-16-16-16zm-64 32c-8.8 0-16 7.2-16 16l0 272c0 8.8 7.2 16 16 16s16-7.2 16-16l0-272c0-8.8-7.2-16-16-16zm128 0c-8.8 0-16 7.2-16 16l0 272c0 8.8 7.2 16 16 16s16-7.2 16-16l0-272c0-8.8-7.2-16-16-16z"
      />
    </svg>
  );
}

// =============================================================================
// ACCESS CODE ICONS (Duotone)
// =============================================================================

// Hashtag Lock (FA Duotone Regular) - 640×512
export function FaHashtagLock({
  secondaryOpacity = 0.4,
  primaryOpacity = 1,
  ...props
}: DuotoneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 640 512"
      fill="currentColor"
      {...props}
    >
      <path
        opacity={secondaryOpacity}
        d=""
      />
      <path
        opacity={primaryOpacity}
        d="M258.9 29c2.8-13-5.5-25.7-18.4-28.5S214.7 6 212 18.9l-23.4 109.1-88.1 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l77.8 0-34.3 160-88 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l77.7 0-21.2 99c-2.8 13 5.5 25.7 18.4 28.5s25.7-5.5 28.5-18.4l23.4-109 155.4 0-21.2 99c-2.8 13 5.5 25.7 18.4 28.5s25.7-5.5 28.5-18.4l4.1-18.9 0-74.2c0-28 12-53.2 31-70.7l1-4.4 0-20.8c0-24.5 6.9-47.3 18.8-66.7l13.1-61.3 88.1 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-77.8 0 21.2-99c2.8-13-5.5-25.7-18.4-28.5S419.2 6 416.4 18.9l-23.4 109.1-155.4 0 21.2-99zM227.4 176l155.4 0-34.3 160-155.4 0 34.3-160zM560 304.1l0 47.9-64 0 0-47.9c0-17.7 14.3-32 32-32s32 14.3 32 32zM416 400l0 96c0 26.5 21.5 48 48 48l128 0c26.5 0 48-21.5 48-48l0-96c0-20.9-13.4-38.7-32-45.3l0-50.6c0-44.2-35.8-80-80-80s-80 35.8-80 80l0 50.6c-18.6 6.6-32 24.4-32 45.3z"
      />
    </svg>
  );
}

// Key (FA Jelly Duo Regular) - 560×560
export function FaKey({
  secondaryOpacity = 0.4,
  primaryOpacity = 1,
  ...props
}: DuotoneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path
        opacity={secondaryOpacity}
        d="M56 415.2L56 476c0 15.5 12.5 28 28 28l60.8 0c9.7 0 16.7-5.1 23.2-11.6l0-44.4c0-15.5 12.5-28 28-28l44.4 0 11.6-11.6 0-44.4c0-15.5 12.5-28 28-28l84 0c45.3 0 79.7-14.3 102.7-37.3S504 241.3 504 196 489.7 116.3 466.7 93.3 409.3 56 364 56 284.3 70.3 261.3 93.3 224 150.7 224 196c0 8.1 .5 15.8 1.3 23.2 1 8.5-1.9 17-8 23.1L64.2 395.4C59 400.6 56 407.8 56 415.2zM448 154a42 42 0 1 1 -84 0 42 42 0 1 1 84 0z"
      />
      <path
        opacity={primaryOpacity}
        d="M406 196a42 42 0 1 0 0-84 42 42 0 1 0 0 84zM364 0C306.6 0 257 18.4 221.7 53.7S168 138.6 168 196c0 5.4 .2 10.7 .5 15.9L24.6 355.8C8.8 371.6 0 392.9 0 415.2L0 476c0 46.4 37.6 84 84 84l60.8 0c22.3 0 43.6-8.9 59.4-24.6l11.6-11.6c5.3-5.3 8.2-12.4 8.2-19.8l0-28 28 0c7.4 0 14.5-3 19.8-8.2l28-28c5.3-5.3 8.2-12.4 8.2-19.8l0-28 56 0c57.4 0 107-18.4 142.3-53.7S560 253.4 560 196 541.6 89 506.3 53.7 421.4 0 364 0zM224 196c0-45.3 14.3-79.7 37.3-102.7S318.7 56 364 56 443.7 70.3 466.7 93.3 504 150.7 504 196 489.7 275.7 466.7 298.7 409.3 336 364 336l-84 0c-15.5 0-28 12.5-28 28l0 44.4-11.6 11.6-44.4 0c-15.5 0-28 12.5-28 28l0 44.4c-6.5 6.5-13.5 11.6-23.2 11.6L84 504c-15.5 0-28-12.5-28-28l0-60.8c0-7.4 3-14.5 8.2-19.8L217.3 242.3c6.1-6.1 9-14.6 8-23.1-.9-7.4-1.3-15.1-1.3-23.2z"
      />
    </svg>
  );
}

// Circle Question (FA Jelly Regular) - 560×560
export function FaCircleQuestion(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 560 560"
      fill="currentColor"
      {...props}
    >
      <path d="M238.5 182.5c-8.4 8.4-14.5 21.8-14.5 41.5 0 15.5-12.5 28-28 28s-28-12.5-28-28c0-31.7 10.2-60.3 31-81s49.4-31 81-31 60.3 10.2 81 31 31 49.3 31 81-10.2 60.3-31 81-49.4 31-81 31c-15.5 0-28-12.5-28-28s12.5-28 28-28c19.6 0 33-6.1 41.5-14.5s14.5-21.8 14.5-41.5-6.1-33-14.5-41.5-21.8-14.5-41.5-14.5-33 6.1-41.5 14.5zM280 448a42 42 0 1 0 0-84 42 42 0 1 0 0 84zM76.5 76.5C126.4 26.5 197 0 280 0S433.6 26.5 483.5 76.5 560 197 560 280 533.5 433.6 483.5 483.5 363 560 280 560 126.4 533.5 76.5 483.5 0 363 0 280 26.5 126.4 76.5 76.5zM116 116c-37.6 37.6-60 93-60 164s22.5 126.4 60 164 93 60 164 60 126.4-22.5 164-60 60-93 60-164-22.5-126.4-60-164-93-60-164-60-126.4 22.5-164 60z" />
    </svg>
  );
}

// Flag (FA Jelly Regular) - 504×560
export function FaFlag(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 504 560"
      fill="currentColor"
      {...props}
    >
      <path d="M168 56C140.4 56 103 64 73.9 71.7 63.7 74.4 56 84.1 56 96.4L56 532c0 15.5-12.5 28-28 28S0 547.5 0 532L0 96.4C0 60.4 23.1 27.2 59.6 17.6 89.1 9.8 132.5 0 168 0 202.7 0 235.9 16.6 261.9 29.6 293 45.2 315.3 56 336 56 353 56 374.1 52.9 395.3 48.6 447.9 37.7 504 75.6 504 133.7l0 189.9c0 35.9-23.1 69.1-59.6 78.8-29.5 7.8-73 17.6-108.4 17.6-35.9 0-70.1-17.8-96.5-31-29.6-14.8-51.4-25-71.5-25-11 0-23.9 1.3-37.6 3.5-15.3 2.4-29.6-8-32-23.3s8-29.6 23.3-32c15.4-2.4 31.4-4.1 46.4-4.1 34.7 0 67.9 16.6 93.9 29.6 31.1 15.6 53.4 26.4 74.1 26.4 27.6 0 65-8 94.1-15.7 10.2-2.7 17.9-12.4 17.9-24.6l0-189.9c0-18.3-19.1-34.9-41.4-30.3-22.6 4.7-48 8.6-70.6 8.6-35.9 0-70.1-17.8-96.5-31-29.6-14.8-51.4-25-71.5-25z" />
    </svg>
  );
}

// =============================================================================
// LEGACY ALIASES (for backward compatibility)
// =============================================================================

export const FaCircleInfo = FaCircleExclamation;
export const FaFilter = FaSliders;
export const FaClipboard = FaCopy;

// =============================================================================
// APP LOGO
// =============================================================================

// LLM Logo - 1080×906
export function LLMLogo(props: IconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1080 906"
      fill="currentColor"
      {...props}
    >
      <polygon points="768 210.11 767.37 209.48 628.89 347.97 490 209.08 490 209 423 209 423 529 490 529 490 304.14 628.88 443.01 628.89 443 628.9 443.01 768 303.91 768 624 835 624 835 209 768 209 768 210.11" />
      <rect x="548" y="555" width="67" height="68" />
      <polygon points="518 555 518 623 422 623 422 589 395 589 395 623 328 623 328 589 301 589 301 623 234 623 234 208 301 208 301 555 328 555 328 208 395 208 395 555 518 555" />
    </svg>
  );
}
